...
public interface SoccerPlayerRepository extends JpaRepository<SoccerPlayer,Long> {
    Optional<SoccerPlayer> findByCodePlayer(String codePlayer);

    boolean existsByCodePlayer(@Pattern(regexp = "^[A-Z]{3}-[0-9]{3}$", message = "Mã cầu thủ phải có định dạng 3 chữ cái viết hoa - 3 số (Ví dụ: GER-001)") String codePlayer);
    @Query("""
